...
/*  Upravil: Kamil Jeřábek, Dominik Harmim, říjen 2017                        */
/* ************************************************************************** */
...
void convert_and_verify(char *infExpr, char *postExprOk)
{
	...
	{ /* string is probably allocated */
		...
		{ /* charater array doesn't contain zero character, this case probably never happend (zero character is often present in unitialized array) */
			printf(
				"ERROR: String appears to be too long (exceeds %d characters).\n   Haven't you forgotten to include zero character?\n\n",
				MAX_LEN - 1
			);
		}
		else
...
int main(int argc, char *argv[])
{
	...
	printf("\n\nC204 - Infix to Postfix Expression Conversion - Advanced Tests\n");
	printf("-----------------------------------------------------------\n\n");

	printf("[TEST01] Complex expression conversion\n");
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	convert_and_verify("(4+8)*(6-5)/((3-2)*(2+2))=", "48+65-*32-22+*/=");

	printf("[TEST02] Multiple braces\n");
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	convert_and_verify("(((((1+1)))))=", "11+=");

	printf("[TEST03] Multiple braces\n");
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	convert_and_verify("(1+y*(2-x/(2+z)))=", "1y2x2z+/-*+=");
